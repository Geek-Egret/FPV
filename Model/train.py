...
init_euler = torch.tensor([[10.0, 20.0, 20.0]], dtype=torch.float, device=device)
...
for i in range(100000):
    geom.step(torch.tensor([[0.0, 0.0, 0.0, 1.0]], dtype=torch.float, device=device), 0.0, True)
    print(geom.drone_pos)
    print(geom.drone_euler)
    print(geom.drone_acc)
    print(geom.drone_vel)
    print(geom.depth.shape)
    print(torch.max(geom.depth))
    print("")
