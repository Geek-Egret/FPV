...
class geom:
    ...
    """
        @ GEOM添加球体(对各个无人机)
        param:球体参数(x,y,z,R):tensor([[x,y,z,R], ...], dtype=torch.float, device=device, requires_grad=True):m
        x,y,z:球体中心位置:m
        R:球体形状:m
    """
    def add_sphere(self, param):
        self._spheres_list.append(param)
        return len(self._spheres_list)-1

    """
        @ GEOM添加圆柱(对各个无人机)
        param:圆柱参数(x,y,z,R,H):tensor([[x,y,z,R,H], ...], dtype=torch.float, device=device, requires_grad=True):m
        x,y,z:圆柱中心位置:m
        R,H:圆柱形状:m
    """
    def add_cylinder(self, param):
        self._cylinders_list.append(param)
    ...
    """
        ...
        black_hole_prob_range:黑洞产生概率范围:{"min": min, "max": max}
    """
    def build(self, show_depth, show_idx, domain_randomization, noise_range, black_hole_prob_range):
        self._show_depth = show_depth
        self._show_idx = show_idx
        self._domain_randomization = domain_randomization
        self._noise_range = noise_range
        self._black_hole_prob_range = black_hole_prob_range
        self._render(show_depth=show_depth, show_idx=show_idx, noise=noise, noise_range=noise_range, black_hole_prob=black_hole_prob)
    ...
    def _sphere_render(self, noise, noise_range, black_hole_prob):
        for i in range(self._batch_size):
            for j in range(len(self._spheres_list[i].shape(0))):
                Rs_xyz = self._depth_pos[i,:].detach().unsqueeze(0).unsqueeze(0).unsqueeze(0)  # 射线起点XYZ
                Rt_xyz = self._pixel_dir[i,:].detach().unsqueeze(0).unsqueeze(0).unsqueeze(0)    # 射线方向向量XYZ
                S_xyz = self._spheres_list[i][j,0:3].detach().unsqueeze(0).unsqueeze(0).unsqueeze(0) # 球体XYZ
                R = self._spheres_list[i][j,3].detach().unsqueeze(0).unsqueeze(0).unsqueeze(0)   # 球体半径
                # 求根公式
