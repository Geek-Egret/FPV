...
class geom:
    ...
    def step(self, pred_euler, pred_thrust):
        for i in range(len(self.drones_list)):
            ...
            # PID控制器计算执行的角速度
            roll_vel = self.roll_pid_list[i].position(current_euler[0], pred_euler[0])*self._dt
            pitch_vel = self.pitch_pid_list[i].position(current_euler[1], pred_euler[1])*self._dt
            yaw_vel = self.yaw_pid_list[i].position(current_euler[2], pred_euler[2])*self._dt
