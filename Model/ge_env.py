...
def main():
    ...
    imu = scene.add_sensor(
        gs.sensors.IMU(
            entity_idx=drone.idx,
            link_idx_local=drone.get_link("base_link").idx_local,
    ...
    init_euler = np.array([0.0, 0.0, 45.0], dtype=np.float32)
    ...
    while(True):
        ...
        # cv2.waitKey(1)
        controller.set_control_target(entity=drone, exp_vx=1.0, exp_vy=0.0, exp_vz=0.0, yaw_rate=0.5)
        rpm = controller.sim_control(0, 3, 2, 1)
        drone.set_propellels_rpm(rpm)
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f"rt_vel_x: {controller.rt_vel_base[0].item():.6f} {controller.rt_vel_world[0].item():.6f}")
        print(f"rt_vel_y: {controller.rt_vel_base[1].item():.6f} {controller.rt_vel_world[1].item():.6f}")
        print(f"rt_vel_z: {controller.rt_vel_base[2].item():.6f} {controller.rt_vel_world[2].item():.6f}")
        print(f"rt_roll: {controller.base_ang[0].item():.6f}")
        print(f"rt_pitch: {controller.base_ang[1].item():.6f}")
        print(f"rt_roll: {controller.base_ang[0].item():.6f}")
        print(f"rt_yaw: {controller.base_ang[2].item():.6f}")
        print(f"exp_vel_x: {controller.exp_vel_base[0]} {controller.exp_vel_world[0]}")
        print(f"exp_vel_y: {controller.exp_vel_base[1]} {controller.exp_vel_world[1]}")
        print(f"exp_vel_z: {controller.exp_vel_base[2]} {controller.exp_vel_world[2]}")
        scene.step()
        # lin_acc = imu.read().lin_acc
        # ang_vel = imu.read().ang_vel
        # print(f"acc: {lin_acc[0]} {lin_acc[1]} {lin_acc[2]}")
        # print(f"ang_vel: {ang_vel[0]} {ang_vel[1]} {ang_vel[2]}")
