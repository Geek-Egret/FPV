...
# 更新位姿
def update_pos(entity, pos_offset=(0.0, 0.0, 0.0), lookat=(1.0, 0.0, 0.0), up=(0.0, 0.0, 1.0)):
    ...
    lookat_tensor = torch.tensor(lookat, dtype=torch.float32, device=device)
    up_tensor = torch.tensor(up, dtype=torch.float32, device=device)
    # 计算相对于world的深度相机位姿
    lookat_world = torch.matmul(R_base_world,  lookat_tensor)
    lookat_world = lookat_world / torch.linalg.norm(lookat_world)    # 归一化
