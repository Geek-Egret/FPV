...
class sim_controller:
    ...
    """
    def __init__(self, drone, frame="world", dt=0.01, base_rpm=14468.429183500699): 
        self.frame = frame          # 参考坐标系（world/base）
        self.KF = drone.KF           # 推力系数
        self.KM = drone.KM           # 力矩系数
        self.mass = drone.get_mass()      # 重量
        ...
        self.kd_vel_roll = 0.0
        self.sigma_limit_vel_roll = 0.02
        self.output_limit_vel_roll = 0.1
        # 左右旋转速度环
        self.kp_vel_yaw = 0.0
        self.ki_vel_yaw = 0.0
        self.kd_vel_yaw = 0.0
    ...
    def set_control_target(self, drone, exp_vx=0.0, exp_vy=0.0, exp_vz=0.0, yaw_rate=0.0):
        self.exp_yaw_rate = yaw_rate
        self.rt_vel_base = drone.get_vel()    # 以base为参考系
        exp_vel = torch.tensor(np.array([exp_vx, exp_vy, exp_vz]), dtype=torch.float32)
        # 获取base坐标系相对于world的四元数、欧拉角和旋转矩阵
        self.base_quat = drone.get_quat()
        self.base_euler = gu.quat_to_xyz(self.base_quat) 
        self.R_base_world = gu.quat_to_R(self.base_quat)
        # 获取base坐标系相对于world的角速率
        self.base_ang = drone.get_ang() # roll pitch yaw
        # 将实时速度转换到world坐标系下
        self.rt_vel_world = torch.matmul(self.R_base_world, self.rt_vel_base)
