...
- CMake配置（无CUDA@Opencv4.10.0）
  ...
  cmake -D CMAKE_BUILD_TYPE=RELEASE \
        ...
        ..
  ```
- CMake配置（X86_64有CUDA@Opencv4.10.0）
  ```
  cmake -D CMAKE_BUILD_TYPE=RELEASE \
        -D CMAKE_INSTALL_PREFIX=/usr/local/opencv-4.10.0 \
        -D CMAKE_CXX_STANDARD=11 \
...
- CMake配置（JETSON@Opencv4.10.0）
  ...
  - 创建OpenCV库路径配置文件 `sudo sh -c 'echo "/usr/local/opencv-4.5.5/lib" > /etc/ld.so.conf.d/opencv-4.5.5.conf'`
## Pangolin编译
...
- 安装 `sudo make install`
## 安装 udev-rules 文件
- 进入 `OrbbecSDK`,以sudo运行 `Script` 下的 `install_udev_rules.sh`
